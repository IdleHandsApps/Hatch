...
public class WebViewConfiguration: WKWebViewConfiguration {
     ...
     */
    func addCSSScript() {
        let script = WKUserScript(source: Injector(cssFilename: "WebView.css").javascriptForCSS(),
    ...
    public func addScript(_ scriptName: String, injectionTime: WKUserScriptInjectionTime) {
        let content = Injector(cssFilename: "WebView.css").stringFromContentInFileName(scriptName)
